BEGIN;
	...
	CREATE TABLE Book_Loans.Book_Status (
        ...
        CONSTRAINT FK_Book_Status_Branch FOREIGN KEY (branch_id) REFERENCES Book_Loans.Library_Branch(branch_id),
		CONSTRAINT FK_Book_Status_Book FOREIGN KEY (book_id) REFERENCES Book_Loans.Book(book_id),
		CONSTRAINT CHK_Book_Status_Current_Condition CHECK (current_condition IN ('fine', 'good', 'fair', 'poor'))
	
	);
	
	CREATE OR REPLACE FUNCTION Book_Loans.Check_Copy_Inconsistencies(_book_id INT, _branch_id INT) RETURNS INT AS $$
	...
	CREATE OR REPLACE FUNCTION Book_Loans.Get_Copies_Inconsistencies() RETURNS SETOF Copy_Inconsistencies AS $$
		BEGIN
			RETURN QUERY 
				SELECT bc.book_id, bc.branch_id, Book_Loans.Check_Copy_Inconsistencies(bc.book_id, bc.branch_id) 
					FROM Book_Copies_Temp AS bc 
					WHERE Book_Loans.Check_Copy_Inconsistencies(bc.book_id, bc.branch_id) <> 0;
		END;
  	...
  	CREATE OR REPLACE PROCEDURE Book_Loans.Fix_Copy_Inconsistencies(_copy Copy_Inconsistencies) LANGUAGE plpgsql AS $$
		...
		BEGIN
			FOR book IN 1.._copy.copies_diff LOOP
                	INSERT INTO Book_Loans.Book_Status (book_id, branch_id) VALUES (_copy.book_id, _copy.branch_id);
			END LOOP;
		END;
	...
	CREATE OR REPLACE PROCEDURE Book_Loans.Add_Copy_Book_Loan(_loan Book_Loans.Book_Loans) LANGUAGE plpgsql AS $$
        BEGIN
            UPDATE Book_Loans.Book_Loans
				SET 
					book_status_id = Book_Loans.Get_Available_Copy_Given_Day(_loan.book_id, _loan.branch_id, _loan.date_out)
				WHERE
